...
| v38 diag | `r_mirrorViewmodel_logBlur` — диагностический dvar: дампит frame-level D3D9 операции (StretchRect, SetRenderTarget(0), do_fullscreen_flip, PSCF c7, опционально per-draw stage-0 texture) в console_mp.log; нужен чтобы найти текстуру/RT через которую engine `r_blur` накладывает «второй слой» неотзеркаленного мира на flipped BB при `r_fullMirror 1`. 0/1/2; cap 400 строк/кадр. Не меняет поведение, только логирование. |
