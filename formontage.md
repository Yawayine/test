# iw3x.dll — гайд для монтажа (актуальная версия v36)

Краткий справочник по mirror-фичам `iw3x.dll` для записи и монтажа фрагов в CoD4 1.7 MP. Включает все актуальные dvar'ы (включая v33 depth-fix, v35 HUD-mirror и **v36 r_fullMirrorDepth**) и рекомендованные пресеты.
...
Подробности и release-сборки upstream: https://github.com/xoxor4d/iw3xo-dev (этот форк = upstream + патчи v32–v36).
...
| `r_mirrorViewmodel_rttTonemapInject` | 0/1, default **1** | Инжектит зеркальный гун в tonemap-source до финального tonemap-прохода → правильный свет / без "песочного" тинта. Оставлять 1. |
...
| `r_fullMirrorDepth` | 0/1, default **1** | **NEW v36.** Дополнительный flip main depth-stencil после каждого color-flip'а `r_fullMirror`. Без него ReShade MXAO/SSAO считает occlusion на исходной (нефлипнутой) геометрии и накладывает на флипнутый color → видно "призрак" гуна и геометрии на стенах в зеркально-неправильных позициях. С ним AO ложится правильно. No-op без ReShade — безопасно держать `1` всегда. |
| `r_hudMirror` | 0/1, default 0 | **NEW v35.** Отдельное зеркало HUD через capture в HUD-RTT + composite с UV-flip. Дизайн: совмещение с ReShade `Flip.fx`. Не трогает мир/гун/post-FX. См. секцию 6 (known limitation). |

### 3.4 Старый matrix-flip путь (legacy — не использовать)
...
**`r_fullMirrorDepth` (v36, NEW).** После каждого `do_fullscreen_flip` запускает 2-pass pixel-shader пайплайн который флипает main depth-stencil:
...
1. **Гун невидимый / black** — проверь `r_mirrorViewmodel_rttBlend` (попробуй 1 или 3 = с alpha-test).
...
6. **Killstreak-баннер с жирной обводкой при `r_hudMirror 1`** — known limitation, переключись на `r_fullMirror 2`.
7. **HUD под ганом** — `r_mirrorViewmodel_rttEarlyComposite 1` (default).
8. **Хочется лог** — `/r_mirrorViewmodel_log 2` для verbose в консоль, `/mirror_dump 3` для file dump.

---

*Этот гайд — для билда `mirror-dll-v3` с патчами v32–v36. Если запускаешь старый билд (без v36) — `r_fullMirrorDepth` не будет существовать; в этом случае MXAO + `r_fullMirror` даст ghost-тени и работающего workaround нет, кроме отключения MXAO.*
